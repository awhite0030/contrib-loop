...
state_prune() {
  ...
  jq -cS --arg cutoff "$cutoff7" '
    .targets //= {}
    | .targets |= with_entries(
        .value.tasks //= {} | .value.posted //= {}
        | .value.tasks |= with_entries(
            if (.value.status == "merged" or .value.status == "no_pr" or .value.status == "failed"
                or .value.status == "validation_failed" or .value.status == "stuck"
