...
# --- 2. (auto-fold removed) -----------------------------------------------------
# Review-fix PRs are folded manually via the fold-fix workflow with explicit
# (fork_pr_number, fix_pr_number) pairs - automatic matching folded unrelated
# PRs into wrong branches (incident 2026-09-19, reverted by force-push).
...
if [ -z "$VALIDATE_TARGET" ]; then
  ...
  for target in $(jq -r 'keys[] | select(. != "globals")' "$CFG_JSON"); do
    ...
    if [ "$(cfg_target "$target" claim_comment)" = "true" ]; then
      if ! gh issue comment -R "$upstream" "$issue" --body "Picking this up - a fix will follow shortly." >/dev/null 2>&1; then
        echo "dispatch[$target]: CANNOT comment on #$issue (blocked or restricted?) - pausing target for today"
        budget_val=$(cfg_target "$target" budget)
        state=$(jq -c --arg t "$target" --arg d "$TODAY" --argjson b "$budget_val" \
          '.dispatchedDay[$t] = $b | .day = $d' <<<"$state")
        continue
      fi
